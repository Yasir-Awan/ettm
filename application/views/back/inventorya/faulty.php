<?php echo form_open(base_url()."inventory/action_on_asset/faulty_do/",array('id' => 'faulty'));?>
         

           <?php 
                foreach($data1 as $row) 
                {
                  if($row[0]['have_sub_items']==1)
                  { ?>
                   <span required="" style="" class="form-control btn-warning mb-5"  >
                   <?php         
                   echo "This Product has Sub Items, when you do it faulty, All it's subitem will become faulty, So Be Aware!";
                   ?> 
                   </span> 
                   <?php              
                  }
