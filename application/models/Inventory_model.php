...
class Inventory_model extends CI_MODEL
{
  ...
  function count_inventory($table)
  {
  ...
  function count_inventory_search($search, $table)
  {
    $this->db->like('name', $search);
    $query = $this->db->get($table);
    return $query->num_rows();
  }
  function allposts_search($limit, $start, $search, $col, $dir, $table)
  {
  ...
  function count_assets($table)
  {
    ...
    if ($this->session->userdata('role') == 5) {
      $this->db->where('route', 5);
      $this->db->group_by('name');
      $query = $this->db->get($table);
      return $query->num_rows();
    }
    if ($this->session->userdata('role') == 3) {
      $this->db->where('route', 3);
      $this->db->group_by('name');
      $query = $this->db->get($table);
      return $query->num_rows();
    }
  }
  function asset_allposts($limit, $start, $col, $dir, $table)
  {
    if ($this->session->userdata('role') == 1 || $this->session->userdata('role') == 2) {
      $this->db->where('route', 1);
      $this->db->group_by('name');
  ...
  function count_assets_search($search, $col, $table)
  {
    $searching_value = '';
    if ($col == 'name') {
      $searching_value =  $this->db->select('*')->like('name', $search)->get('items')->result_array();
    }
    if ($col == 'site') {
      $searching_value =  $this->db->select('*')->like('name', $search)->get('sites')->result_array();
    }
    $this->db->like($col, $searching_value[0]['id']);
    $this->db->group_by('name');
    $query = $this->db->get($table);
    return $query->num_rows();
  }
  function asset_allposts_search($limit, $start, $search, $col, $dir, $table)
  {
    $searching_value = '';
    if ($col == 'name') {
      $searching_value =  $this->db->select('*')->like('name', $search)->get('items')->result_array();
