...
class ApiItemReinstallController extends REST_Controller
{
    ...
    public function index_post($id)
    {
        ...
        if ($token == $this->session->userdata('access_token')) {
            if ($id) {
                ...
                if ($last_transaction_data[0]['is_sub_item'] == 1) {
                    ...
                    foreach ($subitems as $subasset) {
                        ...
                        echo json_encode(array('response' => true, 'message' => 'Reinstalled.', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
                        exit;
                    }
                }
                if ($last_transaction_data[0]['is_sub_item'] == 0) {
                    ...
                    $data = array(
                        ...
                        'serial_no' => $last_transaction_data[0]['serial_no'],
                        'identification_no' => $last_transaction_data[0]['identification_no'],
                        'unit_repairing_cost' => $this->input->post('repair_price'),
                        'organisation_type' => $last_transaction_data[0]['organisation_type'],
                        'organisation' => $last_transaction_data[0]['organisation'],
                        'organisation_address' => $last_transaction_data[0]['organisation_address'],
                        'repairing_person_type' => $last_transaction_data[0]['repairing_person_type'],
                        'person' => $last_transaction_data[0]['person'],
                        'person_contact' => $last_transaction_data[0]['person_contact'],
                        'is_sub_item' => 0,
                        'transaction_type' => "9",
                        'route' => $last_transaction_data[0]['route'],
                        'site' => $last_transaction_data[0]['site'],
                        'location' => $last_transaction_data[0]['location'],
                        'user_type' => "2",
                        'added_by' => $this->input->post('supervisor_id'),
                        'action_date' => $date,
                        'return_date' => $this->input->post('repair_completion'),
            ...
            $this->response('Unauthorize Access', 401);
        }
    }
}
