...
class Toolplaza extends CI_Controller
{
		...
		if ($para1 == 'list') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				foreach ($_FILES['supporting_file']['name'] as $key => $value) {
					...
					if (!empty($value)) {
						if (!empty($_POST['suppporting_document_name'][$key])) {
							...
							echo json_encode(array('response' => FALSE, 'message' => "One of your supporting document name is missing"));
							exit;
						}
					}
				}
				...
				if ($this->input->post('mtr_type') == 1) {
					...
					if ($check_month) {
						echo json_encode(array('response' => FALSE, 'message' => 'You have already added MTR for this month please delete previous one'));
						exit;
					}
				} else {
				...
				if (empty($_FILES['mtr_file']['name'])) {
					echo json_encode(array('response' => FALSE, 'message' => 'Please upload a valid file'));
					exit;
				}
				...
				if (empty($data['start_date']) || empty($data['end_date'])) {
					echo json_encode(array('response' => FALSE, 'message' => "An Error Occoured Please Refresh Your Page And try Again"));
					exit;
				}
	...
	public function dtr($para1 = '', $para2 = '', $para3 = '')
	{
		...
		if ($para1 == 'list') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($dtr_date) {
					echo json_encode(array('response' => FALSE, 'message' => 'DTR of this date already exists'));
					exit;
				} else {
					foreach ($_FILES['supporting_file']['name'] as $key => $value) {
	...
	public function supportrequest($para1 = '', $para2 = '', $para3 = '')
	{
		if ($para1 == 'list') {
			...
			if (empty($dlt)) {
				echo json_encode(array('response' => FALSE, 'message' => 'Delete Error'));
				exit;
			} else {
	...
	public function dsr($para1 = '', $para2 = '', $para3 = '')
	{
		...
		if ($para1 == 'list') {
			...
			if (!$algorithm_add) {
				echo json_encode(array('response' => FALSE, 'message' => 'DSR entry Failed'));
				exit;
			} else {
			...
			if (!$algorithm_edit) {
				echo json_encode(array('response' => FALSE, 'message' => 'DSR Update Failed'));
				exit;
			} else {
	...
	public function dsr_add_edit($id, $tool, $edit, $page, $para2)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($page == 'C') {
				...
				if ($dsr_date) {
					echo json_encode(array('response' => FALSE, 'message' => 'DSR of this date already exists'));
					exit;
				} else {
			...
			if (isset($dsr_last)) {
				if (isset($edit['north'])) {
					...
					foreach ($edit['north'] as $n) {
						...
						if ($page == 'C') {
							...
							if (!$data_dsr_n_lane) {
								$v = 0;
								$this->dsr_model->delete_dsr($dsr_last[0]['id']);
							}
						}
						if ($page == 'U') {
						...
						if (!$data_dsr_n_lane) {
							...
							if ($page == 'U') {
								$table = 'dsr_lane';
								$where = array('dsr_id' => $para2, 'lane_status' => 1);
							...
							if (isset($dsr_lane_last)) {
								...
								// 			if (!$data_dsr_m2m_inventory_n_lane) {
								// 				$v = 0;
								// 				$this->dsr_model->delete_dsr($dsr_last[0]['id']);
								// 			} else {
								// 				$v = 1;
								...
								// 		if ($page == 'U') {
								// 			$table = 'dsr_m2m_inventory_lane';
								// 			$where = array('dsr_id' => $para2, 'dsr_inventory_id' => $this->input->post('id_' . $inv['abr']));
								...
								echo json_encode(array('response' => FALSE, 'message' => 'DSR Lane was not found'));
								exit;
							}
						}
						$i++;
					...
					foreach ($edit['south'] as $s) {
						...
						if (!$data_dsr_s_lane) {
							...
							if (isset($dsr_lane_last)) {
								...
								// 			if (!$data_dsr_m2m_inventory_s_lane) {
								// 				$v = 0;
								// 				$this->dsr_model->delete_dsr($dsr_last[0]['id']);
								// 			} else {
								// 				$v = 1;
				...
				if (isset($edit['dsr_bound'])) {
					...
					foreach ($edit['dsr_bound'] as $bound) {
						...
						// 			if (!$data_dsr_m2m_inventory_bound) {
						// 				$v = 0;
						// 				$this->dsr_model->delete_dsr($dsr_last[0]['id']);
						// 			} else {
						// 				$v = 1;
						...
						// 		if ($page == 'U') {
						// 			$table = 'dsr_m2m_inventory_bound';
						// 			$where = array('dsr_id' => $para2, 'dsr_bound_id' => $dsr_m2m_inventory_bound_data['dsr_bound_id'], 'dsr_inventory_id' => $dsr_m2m_inventory_bound_data['dsr_inventory_id']);
				...
				// 		if ($page == 'U') {
				// 			$table = 'dsr_m2m_inventory';
				// 			$where = array('dsr_id' => $para2, 'dsr_inventory_id' => $dsr_m2m_inventory_bound_data['dsr_inventory_id']);
				...
				echo json_encode(array('response' => FALSE, 'message' => 'DSR was not found'));
				exit;
			}
			if ($tool == 9) {
				if ($v == 0) {
		...
		if ($para1 == 'list') {
			...
			foreach ($questions as $key1 => $value) {
				...
				if ($this->input->post('quantity_' . $value) < 0 || $this->input->post('price_' . $value) < 0) {
					echo json_encode(array('response' => FALSE, 'message' => 'Negative Values Not Allowed'));
					exit;
				}
	...
	public function traffic_counting($para1 = '', $para2 = '')
	{
		if ($para1 == 'list') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$insert_id = $this->db->insert_id();
				echo json_encode(array('response' => TRUE, 'message' => "Session started", 'is_redirect' => TRUE, 'redirect_url' => base_url() . 'toolplaza/traffic_counting/session_start/' . $insert_id));
				exit;
			}
			...
			if ($this->form_validation->run() == FALSE) {
				...
				echo json_encode(array('response' => TRUE, 'message' => "Session updated successfully", 'is_redirect' => TRUE, 'redirect_url' => base_url() . 'toolplaza/traffic_counting/'));
				exit;
			}
		} elseif ($para1 == 'delete') {
			$this->db->where('id', $para2);
	...
	public function Traffic_Entry($para1 = '', $para2 = '')
	{
		if ($para1 == 'add') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				echo json_encode(array('response' => TRUE, 'message' => "Traffic Added", 'is_redirect' => TRUE, 'redirect_url' => base_url() . 'toolplaza/Traffic_View'));
				exit;
			}
		}
	}
