...
class Supervisor_inventory extends CI_Controller
{
	...
	public function tabs($para1 = '')
	{
		...
		if ($para1 == 'items') {
			...
			return redirect('supervisor_inventory/installed_inventory/list/');
		} elseif ($para1 == 'sites') {
	...
	public function installed_inventory($para1 = '', $para2 = '', $para3 = '')
	{
		...
		if ($para1 == 'list') {
			if ($para2) {
				$this->page_data['installs'] = $this->db->select('*')->where('location', $para2)->order_by('id', 'desc')->get('installed_inventory')->result_array();
			} else {
				$this->page_data['installs'] = $this->db->select('*')->where('site', $this->session->userdata('site'))->order_by('id', 'desc')->get('installed_inventory')->result_array();
			}
			// $this->page_data['installs'] = $this->db->get_where('installed_inventory',array('site' => $this->session->userdata('site')))->result_array();
			$this->page_data['sites'] = $this->Inventory_model->getsites();
			$this->page_data['locations'] = $this->Inventory_model->Site_Locations($this->session->userdata('site'));

			$this->page_data['items'] = $this->Inventory_model->get_Items();
			$this->page_data['asset_transactions'] = $this->Inventory_model->get_asset_transactions();
			$this->load->view('front/supervisor_inventory/display_installed', $this->page_data);
		} elseif ($para1 == 'delete') {
	...
	public function assets($para1 = '', $para2 = '', $para3 = '')
	{
		if ($para1 == 'list') {
			...
			// get_where('assets',array('site' => $this->session->userdata('site')))->result_array();	
			$this->page_data['sites'] = $this->Inventory_model->getsites();
			$this->page_data['items'] = $this->Inventory_model->get_Items();
			$this->page_data['asset_transactions'] = $this->Inventory_model->get_asset_transactions();
			...
			$this->db->where('id', $para2);
			$this->db->delete('assets');
			echo json_encode(array('response' => true, 'message' => 'Deleted successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page/'));
			exit;
		} elseif ($para1 == 'assets_publish_set') {
			$article = $para2;
			if ($para3 == 'true') {
	...
	public function add_item_do()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo json_encode(array('response' => true, 'message' => 'Item Added Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
			exit;
		} else {
	...
	public function edit_item_do($item_id = '')
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->db->update('items', $data);
			echo json_encode(array('response' => true, 'message' => 'Tool plaza updated successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page/'));
			exit;
		} else {
			echo json_encode(array('response' => TRUE, 'message' => validation_errors()));
			exit;
	...
	public function add_asset()
	{
		...
		$this->page_data['sites'] = $this->db->select('*')->where('id', $this->session->userdata('site'))->get('sites')->result_array();
		// $this->Inventory_model->getsites();
		...
		if ($this->form_validation->run() == TRUE) {
			if ($this->session->userdata('supervisor_id')) {
				if ($this->input->post('quantity') > 1) {
					...
					echo json_encode(array('response' => true, 'message' => 'Asset Stock Created Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				} else {
					$description = $this->db->get_where('items', array('id' => $this->input->post('asset_name')))->result_array();
					// $date = date("Y-m-d H:i:s");
					...
					$this->db->insert('notifications', $data11);
					echo json_encode(array('response' => true, 'message' => 'Asset Created Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				}
			}
		} else {
		...
		if ($para1 == 'checkin') {
			...
			if ($this->form_validation->run() == TRUE) {
				if ($this->session->userdata('supervisor_id')) {
					...
					$this->db->insert('notifications', $data11);
					/** Query to insert Action Noification in Table */
					echo json_encode(array('response' => true, 'message' => 'Asset Checked In Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				}
			}
			...
			if ($this->form_validation->run() == TRUE) {
				if ($this->session->userdata('supervisor_id')) {
					...
					$this->db->insert('notifications', $data11);
					/** Query to insert Action Noification in Table */
					echo json_encode(array('response' => true, 'message' => 'Asset Checkedout Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				}
			}
		} elseif ($para1 == 'install') {
			$this->page_data['installs'] = explode(',', $_POST['asset']);
			$assets = $this->page_data['installs'];
			$data = array();
			foreach ($assets as $asset) {
				$data[] = $this->db->get_where('assets', array('id' => $asset))->result_array();
			}
			//	echo "<pre>"; print_r($data); exit;
			$data2 = array();
			foreach ($data as $row) {
			...
			$data3 = array();
			foreach ($assets as $asset) {
				$installing_names = $this->db->get_where('assets', array('id' => $asset))->result_array();
				$this->db->select('assets.name AS temp_id,items.*');
				$this->db->from('assets');
				$this->db->join('items', 'assets.name = items.id');
				$this->db->where('assets.id', $asset);
				$query = $this->db->get();
				$data3[] = $query->result_array();
				$this->page_data['data'] = $data3;
			}
			$this->page_data['data1'] = $data;
			$this->page_data['site'] = $this->db->get_where('sites', array('id' => $this->session->userdata('site')))->result_array();
			$this->page_data['locations'] = $this->db->get_where('locations', array('site' => $this->session->userdata('site')))->result_array();
				...
				if ($this->session->userdata('supervisor_id')) {
					if ($loc_Counter > 1) {
						if ($this->input->post('repairing_company') == "1") {
							...
							echo json_encode(array('response' => true, 'message' => 'Installation Successfull.', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
							exit;
						}
					...
					if ($loc_Counter = 1) {
						if ($this->input->post('repairing_company') == "1") {
							...
							foreach ($asset_ids as $id) {
								$date = date("Y-m-d H:i:s");
								$assets = $this->db->get_where('assets', array('id' => $id))->result_array();
								$installing_data = array(
			...
			$this->load->view('front/supervisor_inventory/faulty', $this->page_data);
			...
			//  $this->form_validation->set_rules('estimated_cost','estimated Cost Price ','required|trim');
			...
			if ($this->form_validation->run() == TRUE) {
				if ($this->session->userdata('supervisor_id')) {
					...
					foreach ($install_ids as $id) {
						...
						$assets_data = array('action_status' => '10', 'site' => $this->input->post('item_site'));
						$this->db->where('id', $repairing_start[0]['asset_id']);
						$this->db->update('assets', $assets_data);
						$this->db->insert('asset_transaction', $data);
					}

					echo json_encode(array('response' => true, 'message' => 'Repairing Completed.', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				}
			}
				...
				if ($this->session->userdata('supervisor_id')) {
					...
					if ($this->input->post('repair_type') == 2) {
						...
						foreach ($install_ids as $id) {
							...
							// echo "<pre>"; print_r($data); exit;
							$assets_data = array('action_status' => '4', 'user_type' => "2", 'checkin_by' => $this->session->userdata('supervisor_id'), 'site' => $this->input->post('item_site'));
						...
						echo json_encode(array('response' => true, 'message' => 'Repairing Starts.', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
						exit;
					}
			...
			foreach ($data as $row) {
				...
				if ($row[0]['transaction_type'] == "3") {
					echo "Installed item Selected.Only Repairing Mode items you can select.";
					exit;
				}
				if ($row[0]['transaction_type'] == "5") {
			...
			if ($this->form_validation->run() == TRUE) {
				if ($this->session->userdata('supervisor_id')) {
					...
					foreach ($asset_ids as $id) {
						...
						$assets_data = array(
							'action_status' => '9', 'site' => $this->input->post('item_site'),
							'user_type' => "2", 'checkin_by' => $this->session->userdata('supervisor_id')
						);
						$this->db->where('id', $repairing_start[0]['asset_id']);
						$this->db->update('assets', $assets_data);
						$this->db->insert('asset_transaction', $data);
			...
			if ($this->form_validation->run() == TRUE) {
				if ($this->session->userdata('supervisor_id')) {
					...
					foreach ($install_ids as $id) {
						...
						//echo "<pre>"; print_r($data); exit;
						$assets_data = array('action_status' => '6', 'user_type' => "2", 'checkin_by' => $this->session->userdata('supervisor_id'), 'site' => $this->input->post('site_id'));
					...
					echo json_encode(array('response' => true, 'message' => 'Retired.', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
					exit;
				}
			}
	...
	public function edit_site_do($site_id = '')
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->db->update('sites', $data);
			echo json_encode(array('response' => true, 'message' => 'Site updated successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page/'));
			exit;
		} else {
	...
	public function add_site_do()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo json_encode(array('response' => true, 'message' => 'Site Added Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
			exit;
		} else {
			echo json_encode(array('response' => TRUE, 'message' => validation_errors()));
			exit;
	...
	public function add_supplier_do()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo json_encode(array('response' => true, 'message' => 'Supplier Added Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
			exit;
		} else {
			echo json_encode(array('response' => TRUE, 'message' => validation_errors()));
			exit;
	...
	public function edit_supplier_do($supplier_id = '')
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->db->update('suppliers', $data);
			echo json_encode(array('response' => true, 'message' => 'Supplier updated successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page/'));
			exit;
		} else {
	...
	public function add_tsps_do()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo json_encode(array('response' => true, 'message' => 'TSP Added Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
			exit;
		} else {
			echo json_encode(array('response' => TRUE, 'message' => validation_errors()));
			exit;
	...
	public function add_manufacturer_do()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			echo json_encode(array('response' => true, 'message' => 'Manufacturer Added Successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page'));
			exit;
		} else {
			echo json_encode(array('response' => TRUE, 'message' => validation_errors()));
			exit;
	...
	public function edit_manufacturer_do($manufacturer_id = '')
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->db->update('manufacturers', $data);
			echo json_encode(array('response' => true, 'message' => 'Manufacturer updated successfully', 'is_redirect' => True, 'redirect_url' => base_url() . 'supervisor_inventory/first_page/'));
			exit;
		} else {
	...
	public function filterby_location($para1 = '')
	{
		$this->page_data['installs'] = $this->db->select('*')->where('location', $para1)->order_by('id', 'desc')->get('installed_inventory')->result_array();
		// $this->page_data['installs'] = $this->db->get_where('installed_inventory',array('site' => $this->session->userdata('site')))->result_array();
		$this->page_data['sites'] = $this->Inventory_model->getsites();
		$this->page_data['locations'] = $this->Inventory_model->Site_Locations($this->session->userdata('site'));
		$this->page_data['items'] = $this->Inventory_model->get_Items();
	...
	public function item_faulty_form($id = '')
	{
		$data = $this->db->get_where('installed_inventory', array('id' => $id))->result_array();
		$this->page_data['identification_no'] = $data[0]['identification_no'];
		$data2 = array();
		foreach ($data as $row) {
			// if($row[0]['transaction_type'] !== 1 && $row[0]['transaction_type'] !== 4 && $row[0]['transaction_type'] !== 5 && $row[0]['transaction_type'] !== 6)
			// {
			...
			if ($row['transaction_type'] == "5") {
				echo "Selected item's repairing recently completed but not installed yet";
				exit;
			}
		...
		$data3 = array();
		foreach ($data as $repair) {
			$installing_names = $this->db->get_where('installed_inventory', array('id' => $data[0]['id']))->result_array();

			$this->db->select('installed_inventory.name AS temp_id,items.*');
			$this->db->from('installed_inventory');
			$this->db->join('items', 'installed_inventory.name = items.id');
			$this->db->where('installed_inventory.id', $repair['id']);
			$query = $this->db->get();
			$data3[] = $query->result_array();
			$this->page_data['data'] = $data3;
		}
		...
		foreach ($data as $repair) {
			$installed_location = $this->db->get_where('installed_inventory', array('id' => $repair['id'], 'transaction_type' => 3))->result_array();
	...
	public function item_repair_form($id = '')
	{
		// echo "<pre>";
		// print_r($_POST);
		// exit;
		...
		foreach ($data as $row) {
			...
			if ($row['transaction_type'] == "9") {
				echo "item should be faulty before putting it in repairing mode.";
				exit;
			}
			if ($row['transaction_type'] == "5") {
				echo "Selected item's repairing recently completed but not reinstalled.";
				exit;
			}
			if ($row['transaction_type'] == "6") {
				echo "Retire item cannot be repair.";
		...
		$data3 = array();
		foreach ($data as $repair) {
			// $installing_names = $this->db->get_where('installed_inventory', array('id' => $repair['id']))->result_array();
			$this->db->select('installed_inventory.name AS temp_id,items.*');
			$this->db->from('installed_inventory');
			$this->db->join('items', 'installed_inventory.name = items.id');
			$this->db->where('installed_inventory.id', $repair['id']);
		...
		foreach ($data as $repair) {
			$installed_location = $this->db->get_where('installed_inventory', array('id' => $repair['id'], 'transaction_type' => $repair['transaction_type']))->result_array();
			$locationNames = $this->db->get_where('locations', array('id' => $installed_location[0]['location']))->result_array();
			$siteNames = $this->db->get_where('sites', array('id' => $locationNames[0]['site']))->result_array();
			$this->page_data['sites'] = $siteNames;
			$this->page_data['locations'] = $locationNames;
	...
	public function item_reinstall_form($id = '')
	{
		...
		$quantity++;
		$data = $this->db->get_where('installed_inventory', array('id' => $id))->result_array();
		$id_no = $data[0]['identification_no'];
		// $locations = $this->db->get_where('asset_transaction', array('installed_id' => $id, 'transaction_type' => "4", 'site' => $this->session->userdata('site')))->result_array();
		...
		foreach ($data as $row) {
			...
			if ($row['transaction_type'] == "9") {
				echo "Item should be in repairing Mode before reinstalling it.";
				exit;
			}
		...
		foreach ($data as $repair) {
			...
			$locationNames = $this->db->get_where('locations', array('id' => $repair['location']))->result_array();
			$siteNames = $this->db->get_where('sites', array('id' => $repair['site']))->result_array();
			$this->page_data['sites'] = $siteNames;
			$this->page_data['locations'] = $locationNames;
		}
		...
		foreach ($data as $repair) {
			...
			$query = $this->db->get();
			$data3 = $query->result_array();
			$this->page_data['data'] = $data3[0]['name'];
		}

		$this->page_data['data1'] = $data;
		$this->page_data['id'] = $id;
		$this->page_data['id_no'] = $id_no;
		$this->page_data['supervisor_id'] = $this->session->userdata('supervisor_id');
	...
	public function item_retire_form($id = '')
	{
		$data = $this->db->get_where('installed_inventory', array('id' => $id))->result_array();
		...
		foreach ($data as $item) {
			...
			$this->db->from('installed_inventory');
			$this->db->join('items', 'installed_inventory.name = items.id');
			$this->db->where('installed_inventory.id', $item['id']);
			$query = $this->db->get();
			$item_join = $query->result_array();
			$this->page_data['data'] = $item_join[0]['name'];
		}
		// $this->page_data['data'] = $data3;
		$this->page_data['id'] = $id;
		$this->page_data['supervisor_id'] = $this->session->userdata('supervisor_id');
