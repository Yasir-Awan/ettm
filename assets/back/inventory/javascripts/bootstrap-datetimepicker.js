(function($) {
    ...
    DateTimePicker.prototype = {
        ...
        init: function(element, options) {
            ...
            var timeFormat = $('meta[name=time-picker-format]').attr('content');
            if (timeFormat && timeFormat === '12-hour') {
            ...
            this.startTime = "";
            this.endTime   = "";
            if (this.$element.data('associated-business-day')) {
              var dateDom = $('#' + this.$element.data('associated-business-day'));
              var businessHours = dateDom.data('date-businesshours');
              if (businessHours) {
                var day = dateDom.data('datepicker').date.getDay();
                var businessHour = businessHours[day];
                if (businessHour) {
                  this.setStartAndEndTimes(businessHour.start_time, businessHour.end_time);
                }
              }
            }
            this.fillDow();
        ...
        setStartAndEndTimes: function(startTime, endTime) {
        ...
        getStartingTimeFrame: function() {
          var startingHour = parseInt(this.startTime.split(':')[0]);
          var startingMinute = parseInt(this.startTime.split(':')[1]);

          return startingHour * 60 + startingMinute;
        },

        getEndingTimeFrame: function() {
          var endingHour   = parseInt(this.endTime.split(':')[0]);
          var endingMinute = parseInt(this.endTime.split(':')[1]);

          return endingHour * 60 + endingMinute;
        },

        getCurrentTimeFrame: function() {
          return this._date.getUTCHours() * 60 + this._date.getUTCMinutes();
        },
            ...
            if (this.options.pick12HourFormat) {
                ...
                for (var i = 0; i < 3; i += 1) {
                    ...
                    for (var j = 0; j < 4; j += 1) {
                        ...
                        if ((startingHour && (parseInt(c) < startingHour)) ||
                            (endingHour && (parseInt(c) > endingHour))) {
                          html += ' disabled';
                        }
                        html += '">' + padLeft(c, 2, "0") + "</td>";
                        current++
                    }
                    html += "</tr>"
                }
        ...
        actions: {
            incrementHours: function(e) {
                ...
                var isInStartingBound = startingHour && (this._date.getUTCHours() >= startingHour);
                var isInEndingBound = endingHour && (this._date.getUTCHours() < endingHour);

                if (isInStartingBound && isInEndingBound) {
                  this._date.setUTCHours(this._date.getUTCHours() + 1)
                }

                var endingTimeFrame   = this.getEndingTimeFrame(),
                  currentTimeFrame  = this.getCurrentTimeFrame();

                if (endingTimeFrame && (currentTimeFrame > endingTimeFrame)) {
                  this._date.setUTCMinutes(parseInt(this.endTime.split(':')[1]));
                }
            },
            incrementMinutes: function(e) {
                var startingTimeFrame = this.getStartingTimeFrame(),
                    endingTimeFrame   = this.getEndingTimeFrame(),
                    currentTimeFrame  = this.getCurrentTimeFrame();

                if (!startingTimeFrame || !endingTimeFrame) {
                  this._date.setUTCMinutes(this._date.getUTCMinutes() + 1);
                  return;
                }

                if ((startingTimeFrame && (currentTimeFrame >= startingTimeFrame)) &&
                     (endingTimeFrame && (currentTimeFrame < endingTimeFrame))) {
                  this._date.setUTCMinutes(this._date.getUTCMinutes() + 1)
                }
            },
            ...
            decrementHours: function(e) {
                ...
                if (startingTimeFrame && (currentTimeFrame < startingTimeFrame)) {
                  this._date.setUTCMinutes(parseInt(this.startTime.split(':')[1]));
                }
            },
            ...
            selectMinute: function(e) {
                ...
                var endingHour = parseInt(this.endTime.split(':')[0]);
                var endingMinute = parseInt(this.endTime.split(':')[1]);
                var currentHour = this._date.getUTCHours();
                var value = parseInt(tgt.text(), 10);
                if ((currentHour === startingHour) && (value < startingMinute) ||
                    (currentHour === endingHour) && (value > endingMinute)) {
                  alert('The selected value does not lie within business timings');
                }
                else {
