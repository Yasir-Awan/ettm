...
function SignaturePad (selector, options) {
  ...
  function drawLine (e, newYOffset) {
    ...
    if (typeof e.targetTouches !== 'undefined') {
      //in case of mobile zoom adjust offset
   ...
   */
  function StartSign(signaturePad){
    ...
    executeForFirstCall = 0;
    var canvasObj = signaturePad.find('#canvas-area')[0];
    setGlobalData('canvas-area', canvasObj);
  ...
  { 
    ...
    var ctx = getGlobalData('canvas-area').getContext('2d');

    if (obj.errorMsg != null && executeForFirstCall == 1 && obj.errorMsg != "" && obj.errorMsg!="undefined")
    {
      alert(obj.errorMsg);
    }
    else
    {
      if (obj.isSigned)
      {
        ...
        setGlobalData('canvas-area', null);
        $('.output').val(obj.imageData);
      }
    }
  }
  ...
  function bindClearCall(){
    $(settings.clear, context).bind('click.signaturepad', function (e) { e.preventDefault(); clearCanvas() });
  }
  
  function styleDrawBoard(){
    drawSigLine();
    $(settings.drawIt, context).addClass(settings.currentClass)
    $(settings.sig, context).addClass(settings.currentClass)
    $(settings.drawItDesc, context).show()
  ...
  function alertUserForPadSignature(){
    if(!$('#topaz-check-box').is(':checked')){
      if ($('.output').val() == ''){
        alert('Please click draw button to capture signature using topaz signature pad');
      } else{
        alert('Please click draw to capture the signature again using topaz signature pad');
      }
    }
  }
  ...
  $('body').on('change', '#topaz-check-box', function(e){
    if ($(this).is(':checked')){
      $('.drawIt').unbind();
      $('#canvas-area').unbind();
      $('.topaz-check-box-class').prop('checked', true);
      $('.drawIt a').removeClass('btn-primary');
      drawIt();
    } else {
      clearCanvas();
      $('.drawIt').unbind();
      $('.pad').unbind();
      $('.topaz-check-box-class').prop('checked', false);
      $('.drawIt a').addClass('btn-primary');
      initializeSignaturePadForTopaz();
    }
  });

  $('#topaz-check-box-no-action').on('change', function(e){
    if ($(this).is(':checked')){
      if ($(this).closest('#topaz-check-box-div').attr('data-visited') == 'false'){  
        $(this).closest('#signature-pad').find('.drawIt').unbind();
        $(this).closest('#signature-pad').find('#canvas-area').unbind();
        $('.drawIt a').removeClass('btn-primary');
        $('.topaz-check-box-class').prop('checked', true);
        drawIt();
        $(this).closest('#topaz-check-box-div').attr('data-visited', 'true');
      }
    } else {
      clearCanvas();
      if($(this).closest('#topaz-check-box-div').attr('data-visited') == 'true'){
        $('.drawIt').unbind();
        $('#canvas-area').unbind();
        $('.topaz-check-box-class').prop('checked', false);
        $('.drawIt a').addClass('btn-primary');
        initializeSignaturePadForTopaz();
        $(this).closest('#topaz-check-box-div').attr('data-visited', 'false');
      }
    }
  ...
  $('.modal').on('shown.bs.modal', function () {
    if ($(this).find('#signature-pad').attr('data-enable-topaz-signature-pad') == 'true'){
      clearCanvas();
      $(this).find('#signature-pad #topaz-check-box').prop('checked', false);
      $('.drawIt').unbind();
      $('.pad').unbind();
      $('.topaz-check-box-class').prop('checked', false);
      $('.drawIt a').addClass('btn-primary');
  ...
  function initializeSignaturePadForTopaz(){
    styleDrawBoard(); //for styling the draw board i.e. applying the classes as before
    bindClearCall();  //bind the clear link to clear canvas in this case too
          //bind the draw click to the topaz signature pad provided function in this case
    $(".drawIt").click(function(e){
      e.stopImmediatePropagation();
      StartSign($(this).closest('#signature-pad'));
    });

    $('body').on('click', '#canvas-area', function(e){
      e.stopImmediatePropagation();
      alertUserForPadSignature();
    });
  }
  ...
  function init () {
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
        ...
        if ((settings.drawOnly || settings.defaultAction === 'drawIt') && $('.sigPad').attr('data-enable-topaz-signature-pad') == 'false') {
          drawIt();
        } else if($('.sigPad').attr('data-enable-topaz-signature-pad') == 'true') {
          initializeSignaturePadForTopaz();
        } else {
          typeIt();
        }
...
$.fn.signaturePad.defaults = {
  ...
  , lineTop : 107 // Distance to draw the line from the top
